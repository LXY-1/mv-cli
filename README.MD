...
- 简单搭建过程
关于从0搭建自动化单元测试环境，可以看另一个github项目[link](),这里主要不同是它集成在项目里面作为项目框架的单元测试工具(不是很完善，但是vue项目基本够用，支持js模块以及.vue组件单元测试，只要会编写单元测试即可以)，除了支持使用es6\7\8\9这些特性编写的js模块进行单元测试之外，还支持.vue组件进行单元测试
- 踩坑
